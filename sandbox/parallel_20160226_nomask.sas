...
%macro parallel(macro=,param_dset=,stagger=0, cleanup=Y, allmail=Y, max_progs=0);
	...
	* this .py creates a sas file in the directory with only the macros from the original program no other parts of the program-- called &maconly_prog.;
	...
	*************** loop through each row param_dset file ;
	...
	%do i = 1 %to &_numrows.;
		...
		* put the values for parameters into macro variable;
		data _NULL_;
			set &param_dset.(firstobs=&i. obs=&i.);
			array _n {*} _NUMERIC_;
		...
		%let macro_call=&macro.(&_macro_params.);
